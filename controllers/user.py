...
@router.post("/create", response_model=user_model.CreatedUser, response_model_exclude_none=True)
def create_user(user: user_model.CreateUser, db: CMySQLConnection = Depends(database_service.connect_db)):
...
def login_user(
    ...
    user: user_model.LoggedInUser = database_service.get_user_by_email(request.username, db=db)

    if not authentication_service.Hash.verify(request.password, user.password):
        raise HTTPException(detail="Incorrect password", status_code=status.HTTP_401_UNAUTHORIZED)
...
def save_recipe(
    ...
    database_service.save_recipe(user_id=current_user.id, recipe_id=recipe.recipe_id, db=db)
    return True
...
def unsave_recipe(
    ...
    return database_service.unsave_recipe(user_id=current_user.id, recipe_id=recipe.recipe_id, db=db)
